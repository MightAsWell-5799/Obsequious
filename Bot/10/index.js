	...
	switch (command) {
		...
		case "rn":
			...
			try {
				...
				message.channel.send("Couldn't change your role name. This might be because it's too long or contains invalid characters.");
			}
			break;
		case "rolecolor":
		case "rc":
		...
		case "cn":
			...
			try {
				...
				message.channel.send("Couldn't change your channel name. This might be because it's too long or contains invalid characters.");
			}
			break;
		case "lock":
			var current = await User.findOne({ ID: message.author.id });
			...
			try {
				...
				var newPerms = channel.permissionOverwrites.array().concat([{ id: message.mentions.members.first().id, deny: ["CONNECT"] }]);
		...
		case "unlock":
			...
			try {
				var channel = await message.guild.channels.cache.get(current.channel);
				var newPerms = channel.permissionOverwrites.array().concat([{ id: message.mentions.members.first().id, allow: ["CONNECT"] }]);
				channel.overwritePermissions(newPerms);
